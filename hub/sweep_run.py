...
def process_candidate(brain_dir, candidate, block_path, trust):
    ...
    if err:
        # BOTH sides of this line matter. quarantine_reason(err) separates
        # `empty-block` from `parse-error` (P3) — a caller that emitted nothing
        # is a different diagnosis and a different fix from a malformed block —
        # and item_path=None is explicit (P8) because `block` is None here: a
        # parse error named no file, so there is no item to annotate.
