...
| **Merge / republish path** | A defective merge destroys data; a republish leaks a secret cross-platform | Backup tag (`pre-merge/<YYYYMMDD-HHMMSS>`, one per run, never force-moved) before every reconciliation; **secret-scrub MUST pass before any merge to `main` AND before any republish** — both gates fail closed; rollback = `git reset --hard <the tag named in that run's report>`; a cross-provider write to an item a cloud caller doesn't own is reverted by the provenance gate |
